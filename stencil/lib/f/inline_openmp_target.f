...
!> @date 2020-07-19
!> @licence LGPL-3.0
subroutine diffuse_inline_openmp_target(in_field, out_field, nx, ny, nz, num_halo, alpha, num_iter) bind(C)
    ...
    do iter = 1, num_iter
        ...
        !$omp   default(none) &
        !$omp   shared(iter, nx, ny, nz, num_halo, alpha_20, alpha_08, alpha_02, alpha_01) &
        !$omp   shared(in_field_, out_field_) &
        !$omp   private(i, j, k)
        do k = 1, nz
            !$omp parallel do collapse(2) schedule(static) &
            !$omp   default(none) &
            !$omp   shared(nx, ny, num_halo, alpha_20, alpha_08, alpha_02, alpha_01, k) &
            !$omp   shared(in_field_, out_field_) &
            !$omp   private(i, j)
