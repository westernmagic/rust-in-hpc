...
subroutine diffuse_laplap_openacc(in_field, out_field, nx, ny, nz, num_halo, alpha, num_iter) bind(C)
    ...
    !$acc   create(tmp1_field( nx + 2 * num_halo, ny + 2 * num_halo, nz))
    do iter = 1, num_iter
        !$acc parallel loop gang &
        !$acc   present(in_field_( nx + 2 * num_halo, ny + 2 * num_halo, nz)) &
        !$acc   present(out_field_(nx + 2 * num_halo, ny + 2 * num_halo, nz)) &
        !$acc   present(tmp1_field(nx + 2 * num_halo, ny + 2 * num_halo, nz))
        do k = 1, nz
